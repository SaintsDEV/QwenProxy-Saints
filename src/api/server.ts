  ...
  startPromise = (async () => {
    ...
    if (accounts.length > 0) {
      ...
      void prepareRemainingAccountsInBackground({
        ...
        // Warm Captcha API early so the first signup/login puzzle is solved automatically.
        if (config.accountCreator.enabled && config.accountCreator.captchaApiEnabled) {
          try {
            const { ensureCaptchaApi } = await import(
              "../services/aliyun-captcha-api.ts"
            );
            const ok = await ensureCaptchaApi();
            console.log(
              ok
                ? `🧩 [Server] Captcha API pronta (Aliyun CDP) para cadastro/login automático.`
                : `⚠️  [Server] Captcha API indisponível — cadastro usará vision fallback.`,
            );
          } catch (err) {
            console.warn(
              `⚠️  [Server] Falha ao subir Captcha API: ${
