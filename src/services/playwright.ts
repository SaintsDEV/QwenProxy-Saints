...
async function loginViaUi(
  ...
  try {
    ...
    // Wait for email input (Qwen uses name=email type=text in pt-BR)
        const emailSelector =
          'input[name="email"], input[type="email"], input[placeholder*="Email" i], input[placeholder*="E-mail" i]';
        try {
        ...
        // Do not only press Enter — click explicit submit when present.
        await page
          .locator(
            'button[type="submit"], button:has-text("Continuar"), button:has-text("Continue"), button:has-text("Next"), button:has-text("Próximo")',
          )
          .first()
          .click({ timeout: 3_000 })
          .catch(async () => {
