...
test("config exposes only Playwright/thread-native current auth and context settings", () => {
  ...
  assert.equal(typeof config.accountCreator.captchaApiEnabled, "boolean");
  assert.equal(typeof config.accountCreator.captchaApiUrl, "string");
  assert.equal(typeof config.accountCreator.captchaGesture, "string");
  assert.equal(typeof config.accountCreator.cdpHost, "string");
  assert.ok(Array.isArray(config.accountCreator.captchaApiFallbackWorkdirs));
});
