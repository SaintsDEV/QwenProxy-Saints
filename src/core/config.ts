...
const envSchema = z
  .object({
    ...
    // Automatic account creator (rate-limit / pool empty)
        ...
        // GLM-style Aliyun captcha solver API (CDP). Preferred over in-page vision.
        ACCOUNT_CREATOR_CAPTCHA_API_ENABLED: z.string().default("true"),
        ACCOUNT_CREATOR_CAPTCHA_API: z.string().default("http://127.0.0.1:18787"),
        ACCOUNT_CREATOR_CAPTCHA_API_WORKDIR: z.string().default(""),
        ACCOUNT_CREATOR_CAPTCHA_API_FALLBACK_WORKDIRS: z.string().default(""),
        ACCOUNT_CREATOR_CAPTCHA_API_START_TIMEOUT_MS: z.string().default("30000"),
        ACCOUNT_CREATOR_CAPTCHA_RETRIES: z.string().default("3"),
        ACCOUNT_CREATOR_CAPTCHA_GESTURE: z.string().default("monotonic_soft"),
        ACCOUNT_CREATOR_CAPTCHA_JOB_TIMEOUT_MS: z.string().default("120000"),
        ACCOUNT_CREATOR_CDP_HOST: z.string().default("127.0.0.1"),
...
export const config = {
  ...
  accountCreator: {
      ...
      captchaApiEnabled: env.ACCOUNT_CREATOR_CAPTCHA_API_ENABLED !== "false",
      captchaApiUrl: env.ACCOUNT_CREATOR_CAPTCHA_API,
      captchaApiWorkdir: env.ACCOUNT_CREATOR_CAPTCHA_API_WORKDIR,
      captchaApiFallbackWorkdirs: env.ACCOUNT_CREATOR_CAPTCHA_API_FALLBACK_WORKDIRS
        ? env.ACCOUNT_CREATOR_CAPTCHA_API_FALLBACK_WORKDIRS.split(path.delimiter)
            .map((v) => v.trim())
            .filter(Boolean)
        : [],
      captchaApiStartTimeoutMs: Math.max(
        5_000,
        parseInt(env.ACCOUNT_CREATOR_CAPTCHA_API_START_TIMEOUT_MS),
      ),
      captchaRetries: Math.max(1, parseInt(env.ACCOUNT_CREATOR_CAPTCHA_RETRIES)),
      captchaGesture: env.ACCOUNT_CREATOR_CAPTCHA_GESTURE || "monotonic_soft",
      captchaJobTimeoutMs: Math.max(
        15_000,
        parseInt(env.ACCOUNT_CREATOR_CAPTCHA_JOB_TIMEOUT_MS),
      ),
      cdpHost: env.ACCOUNT_CREATOR_CDP_HOST || "127.0.0.1",
    },
  apiKey: env.API_KEY,
  qwen: {
